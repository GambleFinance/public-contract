...
contract Distributor is Policy {
    ...
    constructor( address _treasury, address _betify, uint32 _epochLength, uint32 _nextEpochTime ) {        
    ...
    function adjust( uint _index ) internal {
        ...
        if ( adjustment.rate != 0 ) {
            ...
            emit LogAdjust(initial, info[ _index ].rate, adjustment.target);
        }
    }
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return IERC20( betify ).totalSupply().mul( _rate ).div( 1000000 );
    }
