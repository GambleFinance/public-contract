...
contract BetifyTreasury is Ownable {
    ...
    /**
        @notice allow approved address to deposit an asset for BETIFY
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BETIFY needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( betify ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BETIFY for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOHMERC20( betify ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( betslip ).balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BETIFY
        @param _amount uint
     */
    function repayDebtWithOHM( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IOHMERC20( betify ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, betify, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( betify ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( betify ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BETIFY valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BETIFY decimals
            value_ = _amount.mul( 10 ** IERC20( betify ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
